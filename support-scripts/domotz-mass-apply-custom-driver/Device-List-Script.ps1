...
foreach ($agent in $filteredAgents) {
    ...
    foreach ($device in $agentDevices) {
        $device | Add-Member -MemberType NoteProperty -Name "agent_name" -Value $agent.display_name -Force
        $device | Add-Member -MemberType NoteProperty -Name "agent_id" -Value $agent.id -Force
        $device | Add-Member -MemberType NoteProperty -Name "agent_status" -Value $agent.status.value -Force
    }
...
$csvData = $filteredDevices | ForEach-Object {
    [PSCustomObject]@{
        'Organization Name' = $_.organization.name
        'Site/Agent Name' = $_.agent_name
        'Domotz Site/Agent ID' = $_.agent_id
        'Device Name' = if ($_.user_data.name) { $_.user_data.name } else { $_.display_name }
        ...
        'Device Make' = if ($_.user_data.vendor) { $_.user_data.vendor } else { $_.vendor }
        'Device Model' = if ($_.user_data.model) { $_.user_data.model } else { $_.model }
